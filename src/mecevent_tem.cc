...
void mecevent_tem (params & p, event & e, nucleus & t, bool cc)
{
	...
	tem_kin (e.in[0].E(), meclepton, mecnucleon, t, central, smearing, bin, kinematics_ok, pb_ok, pb_trials, mc_sampling, mc_strength);
...
void tem_kin (double E, particle &meclep, particle *nucleon, nucleus &t, double mec_central, double mec_smearing, double binding, 
	      bool &kinematics, bool czy_pb, int ile_pb, double sampling, int strength)
{
	...
	for (int jj=0; jj<ile_pb; jj++)//number of trials to satisfy Pauli blocking 
	{
	  ...
	  if (sampling<0.01 && sampling >-0.01)
	    ...
	    {dir_cm = rand_direc (qq, sampling, strength) * sqrt(Ecm * Ecm  - M2);} //preferred direction wrt momentum transfer
