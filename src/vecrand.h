...
inline vec rand_dir2 (vec aa, double kappa);  ///< random unit vector in 3D space restricted by kos; kos = qq * ran / qq.length() std::fabs(kos) < kappa where ran is any random vector.  
inline vec rand_dir3 (vec aa, double kappa);
...
inline vec rand_direc (vec a, double alpha, int degree);
inline vec rand_direc2 (vec a, double alpha, int degree, double kappa); /// random vector according to the fff distribution restricted by the kos. kos = qq * ran / qq.length() std::fabs(kos) < kappa where ran is any random vector with kosine chosen from fff distribution.
inline vec rand_direc3 (vec a, double alpha, int degree, double kappa);
inline double fff (double kos, double par, int n);
inline double fff (double kos, double par, int n, double cosboundary);
...
vec rand_dir ()
{
  ...
  while ((d = xx * xx + yy * yy + zz * zz) > 1 || (d<0.15)); //default (d<0.25) 
  d=sqrt(d);
  return vec (xx/d, yy/d, zz/d);
}
vec rand_dir2 (vec aa, double kappa)
{
  double xx, yy, zz, d;
  double kos = 0;
  vec ran;
  ...
  {
    ran = rand_dir();
    kos = ran * aa / aa.length();
  }while(std::fabs(kos) >= kappa);

  return ran;
}
vec rand_dir3(vec aa, double kappa)
{
  vec ran = rand_dir();
  double kos = (2 * frandom() - 1) * kappa;
  double phi = 2.0 * Pi * frandom();
  
  vec drugi = vecprod(aa, ran);
  vec trzeci= vecprod(aa, drugi);

  vec base1 = aa/aa.length();
  vec base2 = drugi/drugi.length();
  vec base3 = trzeci/trzeci.length();

  vec dir = base1 * kos + base2 * sqrt(1 - std::pow(kos,2) )*cos(phi) + base3*sqrt(1 - std::pow(kos,2) )*sin(phi);
...
double fff (double kos, double par, int n)
{
  if (par>0)
  { return 1 - par + par*std::pow(fabs(kos), n); }
    //return 1 - (1-fabs(kos))*par;
  else
  { //cout<<"redukcja = "<<1 + par*fabs(kos)<<endl;
    return 1 + par*std::pow(std::fabs(kos), n);
    //return 1 + par*fabs(kos);
  }
}
double fff(double kos, double par, int n, double cosboundary)
{
  if (par > 0)
...
vec rand_direc (vec aa, double alpha, int degree)
{
  double spr, los;
  vec ran;
  double xx, yy, zz, d;
...
vec rand_direc2 (vec aa, double alpha, int degree, double kappa)
{
  double spr, los;
  vec ran;
  double xx, yy, zz, d;
  ...
  do   
  {
    ...
    spr = fff (kos, alpha, degree, kappa);
    los = frandom();
    //cout<<"licznik ="<<licznik<<endl;
    //licznik++;
  }
...
vec rand_direc3(vec a, double alpha, int degree, double kappa)
{
  double kos = 0;
  double spr = 0; 
  double los = 0;
  //First sample cos according to the fff
  do {
