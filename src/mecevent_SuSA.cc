...
void mecevent_SuSA (params & p, event & e, nucleus & t, bool cc)
{
  ...
  if(width_q0>0) weight=SuSA_kin_and_weight (e.in[0].E(), meclepton, mecnucleon, t, mec_central,
                                               mec_smearing, binding, ile_pb, mc_sampling, mc_strength);

  e.weight = weight;
  if (weight>0) mec_do_cc (mecnucleon, p.mec_ratio_pp, p.mec_ratio_np);
        ...
        do
        {
          ...
          if(!( ( suma.t < suma.length() ) || ( suma*suma < 4.0 * MN2)))
          {
            ...
            while(PB&&(licz1<ile_PB))
            {
              ...
              {dir_cm = rand_direc (qq, sampling, strength) * sqrt(Ecm * Ecm  - MN2);} // preferred direction wrt momentum transfer
